...
# 1. Multi-Head Self-Attention  (same as ViT)
...
# 3. Patch and Position Embedding
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3, input_dim=3, old=1):
        ...
        # print('img_size', img_size)               # (224, 224)
        # print('grid_size', grid_size)             # (14, 14) for 224*224 input and 16*16 patches, (16, 16) for 256*256 input and 16*16 patches
...
# 4. Transformer Base Block
# 
#  x  ---> norm ---> attention ---> add ---> norm ---> MLP ---> add ---> output
...
# 5. Repeat num_layers Blocks for Transformer of ART
...
# 7. Define a resnet block of ART
...
# 8. Channel Compression after upsampling and concatenation of ART
# 
# x ---> conv 3*3 ---> conv 3*3 ---> concat ---> output
# |__________ conv 1*1 or skip _________|
#
...
# 9. aggregated residual transformer (ART) block
#
#  x  ---> downsample ---> embed ---> transformer ---> upsample ---> concat ---> channel compression ---> resnet block ---> output
...
if __name__ == '__main__':
    ...
    model = ResViT(config, input_dim=1, img_size=224, output_dim=1, vis=True).to('cuda')
    print(f'Parameter count: {sum(p.numel() for p in model.parameters()) / 1e6 :.2f} M') # 123.35 M
