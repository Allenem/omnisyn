...
class TransUNet(nn.Module):
    def __init__(self, img_dim, in_channels, out_channels, head_num, mlp_dim, block_num, patch_dim, class_num):
        ...
        self.act = nn.Sigmoid() if class_num == 1 else nn.Softmax(dim=1)
...
if __name__ == '__main__':
    ...
    print(f'TransUNet params: {sum(p.numel() for p in transunet.parameters()) / 1e6:.2f}M')
    print(transunet(torch.randn(1, 1, 224, 224).to('cuda')).shape)