...
# ---------------- Decoder (No Condition) ---------------- #
class SwinUNETRDecoder_NoCond(nn.Module):
    def __init__(
        self,
        out_channels: int,
        feature_size: int = 24,
    ...
    def forward(self, features: dict):
        dec3 = self.decoder5(features[5], features[4])  # bottleneck + hidden3
...
# ---------------- Wrapper: No Condition ---------------- #
class SwinUNETR_NoCond(nn.Module):
    """
    Encoder + Non-conditioned Decoder
    """
    def __init__(
        self,
        img_size: Sequence[int] | int,
        ...
        self.decoder = SwinUNETRDecoder_NoCond(
            out_channels, feature_size, norm_name, spatial_dims
        )
...
class SwinUNETR_addSigmoid(nn.Module):
    def __init__(self, **kwargs):
        super().__init__()
        self.model = SwinUNETR_NoCond(**kwargs)
        self.act = nn.Sigmoid()
...
swinunetr = SwinUNETR_addSigmoid(
                    img_size=(224, 224),
                    in_channels=1,
                    out_channels=1,
                    feature_size=48,
...
swinunetrv2 = SwinUNETR_addSigmoid(
                    img_size=(224, 224),
                    in_channels=1,
                    out_channels=1,
                    feature_size=48,
...
# class SwinUNETR_LabelCond_addSigmoid(nn.Module):
#     def __init__(self, **kwargs):
#         super().__init__()
#         self.model = SwinUNETR_LabelCond(**kwargs)
#         self.act = nn.Sigmoid()
...
class SwinUNETR_LabelCond_addSigmoid(SwinUNETR_LabelCond):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.act = nn.Sigmoid()

    def forward(self, x: torch.Tensor, label: torch.Tensor):
        out = super().forward(x, label)
        return self.act(out)


swinunetr_LabelCond = SwinUNETR_LabelCond_addSigmoid(
                    img_size=(224, 224),
                    in_channels=1,
                    out_channels=1,
                    feature_size=48,
...
swinunetrv2_LabelCond = SwinUNETR_LabelCond_addSigmoid(
                    img_size=(224, 224),
                    in_channels=1,
                    out_channels=1,
                    feature_size=48,
...
if __name__ == "__main__":
    ...
    model1 = swinunetr.to(device)
    model2 = swinunetrv2.to(device)
    model3 = swinunetr_LabelCond.to(device)
    model4 = swinunetrv2_LabelCond.to(device)
    print(f'Parameters of SwinUNETR: {sum(p.numel() for p in model1.parameters() if p.requires_grad) / 1e6:.2f}M')
    print(f'Parameters of SwinUNETRv2: {sum(p.numel() for p in model2.parameters() if p.requires_grad) / 1e6:.2f}M')
    print(f'Parameters of SwinUNETR_LabelCond: {sum(p.numel() for p in model3.parameters() if p.requires_grad) / 1e6:.2f}M')
    print(f'Parameters of SwinUNETRv2_LabelCond: {sum(p.numel() for p in model4.parameters() if p.requires_grad) / 1e6:.2f}M')
    # Parameters of SwinUNETR: 25.14M
    # Parameters of SwinUNETRv2: 28.66M
    ...
    with torch.no_grad():
        out1 = model1(img)
        out2 = model2(img)
        out3 = model3(img, label)
        out4 = model4(img, label)
    print(out1.shape)
    print(out2.shape)
    print(out3.shape)
    print(out4.shape)
